...
function StatusPill({ children }) {
  ...
  const styles =
    text.includes("cancel") || text.includes("void")
      ...
      : text.includes("complete") || text.includes("paid") || text.includes("delivered")
      ? "bg-emerald-100 text-emerald-700"
      : text.includes("pending") || text.includes("new") || text.includes("processing")
      ? "bg-amber-100 text-amber-700"
      : "bg-slate-100 text-slate-700";
  return (
    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs ${styles}`}>
      {children}
    </span>
  );
}
